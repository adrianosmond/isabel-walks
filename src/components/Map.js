...
class Map extends Component {
  componentDidMount() {
    ...
    this.map.on('load', () => {
      ...
      Promise.all(segments.map((s, idx) => fetch(s.url)
        .then(r => r.json())
        .then((data) => {
          this.map.addLayer({
            id: `route-${idx}`,
            ...
            paint: {
              'line-color': s.complete ? 'rgb(173,29,29)' : 'rgba(173,29,29,0.5)',
        ...
        .then((features) => {
          features.forEach(seg =>
            seg.features[0].geometry.coordinates.forEach(bounds.extend.bind(bounds)));
        })
  ...
  render() {
    return (
      <section className="map" id="map" ref={(el) => { this.mapContainer = el; }}></section>
    );
  }
}
